...
// Defines values for ReadinessFailureStatus.
const (
	NotReady ReadinessFailureStatus = "not ready"
)

// Valid indicates whether the value is a known member of the ReadinessFailureStatus enum.
func (e ReadinessFailureStatus) Valid() bool {
	switch e {
	case NotReady:
		return true
	default:
...
// LivenessStatus Examples: {"status":"ok"}
...
// ReadinessFailure Examples: {"failed":["postgres"],"status":"not ready"}
type ReadinessFailure struct {
	// Failed Names of the dependency checks that failed.
	Failed []string               `json:"failed"`
	Status ReadinessFailureStatus `json:"status"`
}

// ReadinessFailureStatus defines model for ReadinessFailure.Status.
type ReadinessFailureStatus string

// ReadinessStatus Examples: {"status":"ready"}
...
type GetReadiness503JSONResponse ReadinessFailure
