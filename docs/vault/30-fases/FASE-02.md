...
|---|---|---|---|
| `[ ]` `T-02-001` | RED — `WithAuthUser` / `WithAuthLookup` unit tests against a recording `pgx.Tx` stub | PR 1 | 130 |
| `[ ]` `T-02-002` | GREEN — `internal/db/auth.go`: `WithAuthUser`, `WithAuthLookup`, `ErrNoAuthUser`, `NewAuthPool` | PR 1 | 120 |
...
| `[ ]` `T-02-012` | RED — `totp.go` unit tests | PR 8 | 100 |
| `[ ]` `T-02-013` | GREEN — `totp.go` | PR 8 | 90 |
| `[ ]` `T-02-014` | RED — `token.go` (JWT) unit tests | PR 9 | 120 |
| `[ ]` `T-02-015` | GREEN — `token.go` | PR 9 | 100 |
| `[ ]` `T-02-016` | Migration `00014_totp_recovery_codes.sql` + catalog/query + recovery-code scenarios | PR 4 | 160 |
