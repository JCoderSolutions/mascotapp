...
- [x] **ARRASTRADA A FASE 02 (T-01-035).** **Un refugio puede asignar un caso a un miembro que NO PUEDE LEER (hallazgo de T-01-028).** `member_visible_users` filtra por `status = 'active'` (arreglo de T-01-016 tras Judgment Day), pero la clave compuesta de `assigned_to_user_id` solo puede chequear que el par de `memberships` **exista**. Y eso es PostgreSQL, verificado en 17 y no supuesto: **una foreign key no puede referenciar un índice único parcial** (`there is no unique constraint matching given keys`), así que `UNIQUE (user_id, shelter_id) WHERE status = 'active'` no está disponible como clave referenciada. Queda `asignable = la fila existe` contra `legible = la fila existe Y está activa`, así que un caso puede terminar en la cola de alguien cuyo nombre ese refugio ya no puede renderizar.
